...
import {runPipeline} from './functions/utils/manager.ts'
...
var data = {
   confirmation: false,
   platform: {confirmation_platform: false , platform: ''},
   fbx:'',
   materials: [],
   textures: [],
}
...
(async () => {
  const result = await runPipeline(data, [WelcomeMessage, DetectPlatform]);
